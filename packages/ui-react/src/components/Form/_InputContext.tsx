import {
    ChangeEvent,
    ComponentType,
    createContext, HTMLInputTypeAttribute,
    useContext,
    useEffect,
    useState
} from "react";
...
type TUseInputContext = (props: IInputProps & { type: HTMLInputTypeAttribute, }) => IInputContext;
...
export const withInput = <P extends IInputProps &{ type: HTMLInputTypeAttribute }>(Component: ComponentType) => {
    return (props: P) => {
        const context = useInputContext(props)

        return <InputContext.Provider value={context}>
            <Component />
